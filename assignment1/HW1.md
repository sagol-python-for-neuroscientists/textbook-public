...
Submission is done using the `repl.it` interface. In the assignment you should see a `main.py` file which `import`s the others. This means that the `main.py` file uses the functions written in the other files by calling them with their proper input. Thus, your first task is to update the functions' contents in their respective files, and then run them (with some made-up inputs, if needed) from `main.py` by clicking the "Run" button. The output of the functions should be seen in the console.

Note that the submission guidelines for this exercise are unique, as the other assignments will be submitted using GitHub.com.
