...
from hw4.q2.hw4_q2 import *


fname = pathlib.Path("data/populations.txt").resolve()
tests_data_folder = pathlib.Path('tests/tests_data').resolve()
...
def test_largest_species_idx():
    data = pd.read_csv(
        tests_data_folder / "q3_largest.csv", index_col=0, header=None, names=["year", "animal"]
    )
...
def test_largest_species_content():
    data = pd.read_csv(
        tests_data_folder / "q3_largest.csv",
        index_col=0,
        header=None,
        names=["year", "animal"],
        squeeze=True,
...
def test_lynx_idx():
    data = pd.read_csv(
        tests_data_folder / "q3_lynx.csv", index_col=0, header=None, names=["year", "lynx"]
    )
...
def test_mean_columns():
    data = pd.read_csv(tests_data_folder / "q3_mean.csv", index_col=0, header=0)
